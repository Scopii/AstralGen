...
pub const pass1: Pass = .{ .renderImgId = img1.gpuId, .shaderIds = &.{sc.t1Comp.id} }; // clear does not work for compute
pub const pass2: Pass = .{ .renderImgId = img2.gpuId, .shaderIds = &.{ sc.t2Vert.id, sc.t2Frag.id } };
pub const pass3: Pass = .{ .renderImgId = img3.gpuId, .shaderIds = &.{ sc.t3Mesh.id, sc.t3Frag.id } };
pub const pass4: Pass = .{ .renderImgId = img4.gpuId, .shaderIds = &.{ sc.t4Task.id, sc.t4Mesh.id, sc.t4Frag.id } };
pub const pass5: Pass = .{ .renderImgId = img4.gpuId, .shaderIds = &.{ sc.gridTask.id, sc.gridMesh.id, sc.gridFrag.id }, .clear = true };
