...
pub const WindowSys = struct {
    ...
    pub fn update(windowData: *WindowData, state: *const EngineData, windowQueue: *WindowQueue, rendererQueue: *RendererQueue, memoryMan: *MemoryManager) !void {
        for (windowQueue.get()) |windowEvent| {
            switch (windowEvent) {
                .addWindow => |inf| try addWindow(windowData, inf.title, inf.w, inf.h, inf.x, inf.y, inf.resize, inf.viewIds),
            }
        }
        windowQueue.clear();
    ...
    fn addWindow(windowData: *WindowData, title: [:0]const u8, w: c_int, h: c_int, x: c_int, y: c_int, resize: bool, viewIds: [4]?ViewportId) !void {
        ...
        const window = try Window.init(props, vk.VkExtent2D{ .width = @intCast(w), .height = @intCast(h) }, resize, viewIds, title[0..title.len]);
        window.minimize();
