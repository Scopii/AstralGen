...
pub fn main() void {
    ...
    astralGen.setupEntitys() catch |err| {
        std.debug.print("AstralGen failed to setup Entitys Err {}\n", .{err});
        return;
    };

    astralGen.setupResources() catch |err| {
        std.debug.print("AstralGen failed to setup Resources Err {}\n", .{err});
        return;
    };
