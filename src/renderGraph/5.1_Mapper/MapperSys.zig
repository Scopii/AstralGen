...
const PassLifetime = @import("../../renderGraph/components.zig").PassLifetime;
    ...
    pub fn build(
        ...
        while (mapperData.resPassIds.getLength() > 0) {
            ...
            for (mapperData.sharedResources.getConstItems()) |memberKey| {
                ...
                if (lastDesc) |ld| newDesc = try compareResDesc(lastRes, &ld, memberKey, &newDesc, registry);
        ...
        if (rc.FRAME_GRAPH_DEBUG) {
            ...
            inline for (.{ mapperData.prevPersistentGroups, mapperData.prevTransientGroups }, .{ "Persistent", "Transient" }) |groups, label| {
                for (groups.getConstItems(), 0..) |group, i| {
                    ...
                    const rootName = try registry.getResourceName(rootRes);
                    const rootPass = try registry.getPassName(group.rootPass);
            ...
            for (mapperData.persistentGroups.getConstItems(), 0..) |group, i| {
                ...
                for (group.firstMapIndex..group.lastMapIndex + 1, 0..) |mapIndex, counter| {
                    ...
                    const memberName = try registry.getResourceName(memberRes);
                    std.debug.print("     -> {}. {s}\n", .{ counter, memberName });
                }
            }
            // Transient Groups
            ...
            for (mapperData.transientGroupLifetimes.constSlice(), 0..) |groupLifetime, i| {
                ...
                const resName = try registry.getResourceName(groupLifetime.rootResource);
...
fn compareResDesc(key1: ResPassId, desc1: *const ResDesc, key2: ResPassId, desc2: *const ResDesc, registry: *const RenderRegistryData) !ResDesc {
    ...
    return switch (desc1.*) {
        .bufDesc => |d1| .{ .bufDesc = try compareBufDesc(resToBuf(key1), &d1, resToBuf(key2), &desc2.bufDesc, registry) },
        .texDesc => |d1| .{ .texDesc = try compareTexDesc(resToTex(key1), &d1, resToTex(key2), &desc2.texDesc, registry) },
    };
}

fn compareBufDesc(bufId1: BufPassId, bufDesc1: *const BufDesc, bufId2: BufPassId, bufDesc2: *const BufDesc, registry: *const RenderRegistryData) !BufDesc {
    const equal = if (std.meta.eql(bufDesc1.*, bufDesc2.*)) true else false;

    if (equal == false) {
        const bufName1 = try registry.getBufferName(bufId1);
        const bufName2 = try registry.getBufferName(bufId2);
...
fn compareTexDesc(texId1: TexPassId, texDesc1: *const TexDesc, texId2: TexPassId, texDesc2: *const TexDesc, registry: *const RenderRegistryData) !TexDesc {
    ...
    if (equal == false) {
        const texName1 = try registry.getTextureName(texId1);
        const texName2 = try registry.getTextureName(texId2);
