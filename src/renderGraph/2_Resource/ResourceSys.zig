...
pub const ResourceSys = struct {
    pub fn build(resourceData: *ResourceData, accessData: *const AccessData, passData: *const PassData, registry: *const RenderRegistryData) !void {
            ...
            switch (getResTyp(access.input)) {
                .Buf => {
                    ...
                    if (resourceData.bufDescs.isKeyUsed(inputBuf) == false) {
                        const desc1 = try registry.getBufferDefinition(inputBuf);
                    ...
                    if (access.output) |output| {
                        ...
                        if (resourceData.bufDescs.isKeyUsed(outputBuf) == false) {
                            const desc2 = try registry.getBufferDefinition(outputBuf);
                ...
                .Tex => {
                    ...
                    if (resourceData.texDescs.isKeyUsed(inputTex) == false) {
                        var desc1 = try registry.getTextureDefinition(inputTex);
                    ...
                    if (access.output) |output| {
                        ...
                        if (resourceData.texDescs.isKeyUsed(outputTex) == false) {
                            var desc2 = try registry.getTextureDefinition(outputTex);
        ...
        if (rc.FRAME_GRAPH_DEBUG) {
            ...
            for (resourceData.memSizes.getConstItems(), 0..) |memSize, i| {
                ...
                const resName = try registry.getResourceName(resPassId);
