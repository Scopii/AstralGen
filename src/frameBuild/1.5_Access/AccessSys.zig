...
pub const AccessSys = struct {
    pub fn buildAccesses(accessData: *AccessData, passData: *const PassData, registryData: *const RegistryData) !void {
        ...
        for (0..passData.activePasses.getLength()) |index| {
                ...
                switch (attribute) {
                    .bufSlot => |bufSlot| {
                        accessData.bufAccesses.append(.{
                            .pass = passId,
                            .input = try registryData.getBufferPassId(bufSlot.bufLink.in),
                            .output = if (bufSlot.bufLink.out) |output| try registryData.getBufferPassId(output) else null,
                    ...
                    .vertexBuffer => |vertexBufSlot| {
                        accessData.bufAccesses.append(.{
                            .pass = passId,
                            .input = try registryData.getBufferPassId(vertexBufSlot.bufInput),
                            .output = null,
                            .access = .read,
                        }) catch return error.PassCoreLinksFull;
                    },
                    .indexBuffer => |indexBufSlot| {
                        accessData.bufAccesses.append(.{
                            .pass = passId,
                            .input = try registryData.getBufferPassId(indexBufSlot.bufInput),
                            .output = null,
                            .access = .read,
                        }) catch return error.PassCoreLinksFull;
                    },
                    ...
                    inline else => |texSlotTypes| {
                        accessData.texAccesses.append(.{
                            .pass = passId,
                            .input = try registryData.getTexturePassId(texSlotTypes.texLink.in),
                            .output = if (texSlotTypes.texLink.out) |output| try registryData.getTexturePassId(output) else null,
            ...
            accessData.passAccessRanges.upsert(passId, accessRanges);
        }
        ...
        if (rc.FRAME_GRAPH_DEBUG) {
            ...
            for (accessData.passAccessRanges.getConstItems(), 0..) |range, i| {
                ...
                std.debug.print(" - Pass Accesses ({s}) (bufIndex {} -> {}) (texIndex {} -> {})\n", .{ pass, range.firstBuf, range.lastBuf, range.firstTex, range.lastTex });
                for (range.firstBuf..range.lastBuf, 0..) |index, counter| {
                    const bufAccess = accessData.bufAccesses.buffer[index];
                    const input = try registryData.getBufferName(bufAccess.input);
                    const output = if (bufAccess.output) |output| try registryData.getBufferName(output) else "null";
                ...
                for (range.firstTex..range.lastTex, 0..) |index, counter| {
                    ...
                    const input = try registryData.getTextureName(texAccess.input);
                    const output = if (texAccess.output) |output| try registryData.getTextureName(output) else "null";
